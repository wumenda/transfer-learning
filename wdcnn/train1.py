...
def accuracy(y_hat, y):
    ...
    y_true = (
        y.clone()
    )  # 必须克隆，否则使用y = y.argmax会修改传入的原值，因为python参数传递是引用
...
class Accumulator:
    ...
    def add(self, *args):
        ...
        ]  # a是原来变量的旧值，b是需要增加的值，a + b就得到增加后总的值
...
class Animator:
    ...
    def __init__(
        ...
        self.X, self.Y, self.fmts = (
            None,
            None,
            fmts,
        )  # X记录横轴数据，Y记录纵轴数据，fmts用于设置线的颜色
    ...
    for epoch in range(num_epochs):
        ...
        for i, (X, y) in enumerate(train_iter):
            ...
            X, y = X.to(device), y.to(
                device
            )  # 将当前batch的数据传入GPU，并且属于for循环里的临时变量，因此每一次for结束都会被清除显存，也就是每次显卡上只有一个batch的数据，这样可以节省显存
            y_hat = net(X)  # 获取预测值
            l = loss(
                y_hat, y
            )  # 计算得到的是一个batch的平均误差,并且要求真实的one_hot标签的数据类型与y_hat相同，那也就意味着y要转成float类型
            l.backward()
            optimizer.step()  # 更新参数
            # 计算当前训练下的训练精度
            with torch.no_grad():
